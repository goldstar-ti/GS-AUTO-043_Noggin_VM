...
# Force standard headers that Bruno/browsers send automatically
headers['Accept'] = 'application/json'
headers['Content-Type'] = 'application/json'
# Many servers block the default 'python-requests' agent; use a custom one
headers['User-Agent'] = 'NogginLCDProcessor/1.0 (Internal Integration)'
...
def make_api_request(url: str, headers: Dict[str, str], tip_value: str,
    ...
    logger.debug(f"DEBUG OUTGOING HEADERS: {json.dumps(headers, default=str)}")

    # Force JSON acceptance which is often required but missing in default requests
    if 'Accept' not in headers:
        headers['Accept'] = 'application/json'
    
    logger.info(f"DEBUG: Request Headers: {json.dumps(headers, indent=2)}")
...
def get_tips_to_process_from_database(limit: int = 10) -> List[Dict[str, Any]]:
    ...
    Priority: failed -> interrupted -> partial -> api_failed -> pending -> csv_imported
    """
    max_retry_attempts: int = config.getint('retry', 'max_retry_attempts')
    ...
    query: str = """
        ...
        WHERE permanently_failed = FALSE
          AND (
              ...
              OR processing_status IN ('pending', 'csv_imported')
          )
        ORDER BY
...
def main() -> int:
    ...
    for i, tip_record in enumerate(tips_to_process, start=1):
        ...
        tip_value: str = tip_record['tip'].strip()
        ...
        current_headers['User-Agent'] = 'NogginLCDProcessor/1.0 (Internal Integration)'
        
        logger.debug(f"DEBUG HEADERS SENT: {json.dumps(current_headers, default=str)}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                noggin_reference: str = response_data.get('lcdInspectionId', 'unknown')
